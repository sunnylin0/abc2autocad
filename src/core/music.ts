...
Abc.prototype.set_sym_glue = function (width) {
    ...
    for (s = tsfirst; s; s = s.ts_next) {
        ...
        if (s.seqst) {
            xmin += s.shrink
            if (xmin > width) {
                error(1, s, "Line too much shrunk 線條收縮太多  $1 $2 $3",
                    xmin.toFixed(1),
                    xx.toFixed(1),
                    width.toFixed(1))
                break
...
Abc.prototype.output_music = function () {
    ...
    (window as { tt123?}).tt123 = voice_tb        //line 自定義
    set_all_sls();  //line 自定義
    toAcadMusicJSON();
...
var slurePos = 0;
/* 字元數：775 */
function set_all_sls() {
    //把 sls 多加 startSlur ,endSlur
    var p_voice, v, s,
        n = voice_tb.length
    for (v = 0; v < n; v++) {
        p_voice = voice_tb[v]
            ...
            for (s = p_voice.sym; s; s = s.next) {
                if (s.sls) {			// slurs from the chord
                    s.sls.forEach((the) => {
                        ...
                        let send = the.se
                        slurePos++;
                        if (!sst?.slurStart) sst.slurStart = []
                        sst.slurStart.push(slurePos)
                        if (!send?.slurEnd) send.slurEnd = []
                        send.slurEnd.push(slurePos)
