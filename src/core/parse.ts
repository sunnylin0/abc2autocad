...
/* じ计1452 */
function slur_add(s: voiceItem, nt?) {		// nt = note if slur ending on note
	var i, s2: voiceItem, sl: SlurGroup;
...
/* じ计1415 */
function do_ties(s: voiceItem, tie_s) {
...
// (possible hook)
/* じ计15188 */
Abc.prototype.new_note = function (grace, sls: SlurGroup[]) {
...
/* じ计760 */
function tp_adj(s: voiceItem, fact) {
	var d,
		tim = s.time,
