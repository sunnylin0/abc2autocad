...
function ly_set(s) {
    var i, j, ly, d, s1, s2, p, w, spw, xx, sz, shift, dw, s3 = s, // start of the current time sequence
    wx = 0, wl = 0, n = 0, dx = 0, a_ly = s.a_ly, align = 0;
...
function draw_lyrics(p_voice, nly, a_h, y, incr) {
    ...
    if (incr > 0) { /* under the staff */
        ...
        for (j = 0; j < nly; j++) {
            y -= a_h[j] * 1.1;
            draw_lyric_line(p_voice, j, y + a_h[j] * .22); // (descent)
        }
        return y / sc;
    }
    /* above the staff */
